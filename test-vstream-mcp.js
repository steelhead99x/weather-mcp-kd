...
/**
 * Test script to verify vstream format is working in MCP server tools
 */

import { weatherMcpServer } from './dist/mcp/weather-server.js';

async function testVStreamMCP() {
  console.log('🧪 Testing vstream format in MCP server...\n');

  try {
    // Test the askWeatherAgentStreamVNext tool
    console.log('1. Testing askWeatherAgentStreamVNext tool...');
    const streamVNextTool = weatherMcpServer.originalTools.ask_weatherAgent_streamVNext;
    ...
    const mainTool = weatherMcpServer.originalTools.ask_weatherAgent;
...
// Run the test
testVStreamMCP().catch(console.error);
